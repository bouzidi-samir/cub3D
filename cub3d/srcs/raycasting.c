...
/*   Updated: 2022/02/11 21:33:41 by samirbouzid      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_draw_distance(t_datastock *datacube)
{
	if (datacube->raycast.side == 0)
		datacube->raycast.perpwalldist = ((double)datacube->raycast.mapx - \
				datacube->raycast.posx + (1 - (double)datacube->raycast.stepx)
				/ 2) / datacube->raycast.raydirx;
	else
		datacube->raycast.perpwalldist = ((double)datacube->raycast.mapy - \
				datacube->raycast.posy + (1 - (double)datacube->raycast.stepy)
				/ 2) / datacube->raycast.raydiry;
	datacube->raycast.lineheight = (int)(datacube->ry
			/ datacube->raycast.perpwalldist);
	datacube->raycast.drawstart = -datacube->raycast.lineheight
		/ 2 + datacube->ry / 2;
	if (datacube->raycast.drawstart < 0)
		datacube->raycast.drawstart = 0;
	datacube->raycast.drawend = datacube->raycast.lineheight
		/ 2 + datacube->ry / 2;
	if (datacube->raycast.drawend >= datacube->ry
		|| datacube->raycast.drawend < 0)
...
void	ft_ray_progress(t_datastock *datacube)
{
	while (datacube->raycast.hit == 0)
	{
		...
		if (datacube->map[datacube->raycast.mapx][datacube->raycast.mapy]
			== '1')
...
void	ft_get_distance(t_datastock *datacube)
{
	if (datacube->raycast.raydirx < 0)
	{
		...
		datacube->raycast.sidedistx = (datacube->raycast.posx
				- datacube->raycast.mapx) * datacube->raycast.deltadistx;
	}
	else
	{
		datacube->raycast.stepx = 1;
		datacube->raycast.sidedistx = (datacube->raycast.mapx + 1.0
				- datacube->raycast.posx) * datacube->raycast.deltadistx;
	}
	if (datacube->raycast.raydiry < 0)
	{
		datacube->raycast.stepy = -1;
		datacube->raycast.sidedisty = (datacube->raycast.posy
				- datacube->raycast.mapy) * datacube->raycast.deltadisty;
	}
	else
	{
		datacube->raycast.stepy = 1;
		datacube->raycast.sidedisty = (datacube->raycast.mapy + 1.0
				- datacube->raycast.posy) * datacube->raycast.deltadisty;
	}
...
int	ft_raycasting(t_datastock *datacube)
{
	...
	mlx_put_image_to_window(datacube->mlx_ptr, datacube->mlx_win,
		datacube->mini.img_player, datacube->mini.posx, datacube->mini.posy);
	ft_swap(datacube);
	return (0);
}
