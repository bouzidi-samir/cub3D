...
/*   Updated: 2022/02/11 14:50:46 by samirbouzid      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_map(char *str, t_datastock *datacube)
{
	...
	j = 0;
	datacube->map[i] = NULL;
	datacube->map[i] = malloc(sizeof(char) * datacube->width + 1);
	if (!datacube->map[i])
		return (0);
	while (str[j] != '\0')
	{
...
// Je check si ma map contient les bon char et si elle est bien à la fin 
// je recup sa largeur et sa hauteur 
...
int	parsing_map(char *fichier, t_datastock *datacube)
{
	...
	datacube->map = malloc(sizeof(char *) * datacube->height);
	while (ret != 0)
	{
		...
		datacube->inmap = check_character(str, datacube);
		if (datacube->inmap == 1)
		{
			datacube->count++;
