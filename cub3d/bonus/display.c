...
/*   Updated: 2022/02/11 19:04:48 by samirbouzid      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_init_texture(t_datastock *datacube)
{
	...
	if (datacube->raycast.side == 0)
		datacube->t.wallx = datacube->raycast.posy
			+ datacube->raycast.perpwalldist * datacube->raycast.raydiry;
	else
		datacube->t.wallx = datacube->raycast.posx
			+ datacube->raycast.perpwalldist * datacube->raycast.raydirx;
...
void	ft_text_draw(t_datastock *datacube, int x, int y)
{
	...
	datacube->t.step = 1.0 * datacube->text[0].height
		/ datacube->raycast.lineheight;
...
int	ft_col_draw(t_datastock *datacube)
{
	...
	datacube->raycast.lineheight = ((double)HEIGHT * datacube->raycast.ratio)
		/ (datacube->raycast.perpwalldist);
	datacube->raycast.drawstart = datacube->center
		- datacube->raycast.lineheight / 2;
	if (datacube->raycast.drawstart < 0)
		datacube->raycast.drawstart = 0;
	datacube->raycast.drawend = datacube->raycast.lineheight / 2
		+ datacube->center;
